            ...
            if let plant = report.suspectedPlant, plant != .notKnownToxic {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Not identified")
                        .font(.system(size: 15, weight: .semibold, design: .serif))
                        .foregroundStyle(Palette.parchment)
                    Text("Quadrat could not match this to a plant it knows. That is not the same as safe. Most toxic plants are not on its list. Call poison control and describe the plant, or bring a cutting.")
                        .font(.system(size: 12))
...
/// camera screen's cascade, zoom, or quadrat framing. Fewer moving parts in
